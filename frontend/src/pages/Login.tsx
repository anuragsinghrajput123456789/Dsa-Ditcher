...
import { Code, Sparkles, LogIn, ArrowRight } from "lucide-react";
...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
       ...
       toast({ title: "Database Offline", description: "Could not establish connection to authentication server.", variant: "destructive" });
    }
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-[85vh] bg-background p-4 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/4 w-72 h-72 bg-purple-500/10 rounded-full filter blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-pink-500/10 rounded-full filter blur-3xl pointer-events-none"></div>
        ...
        <Card className="glass-card shadow-2xl border border-border/50 rounded-3xl overflow-hidden">
          ...
          <CardHeader className="space-y-3 pt-8 pb-6">
            <div className="flex justify-center mb-1">
              <div className="w-14 h-14 bg-gradient-to-tr from-blue-600 to-purple-600 dark:from-blue-500 dark:to-purple-500 rounded-2xl flex items-center justify-center shadow-lg shadow-purple-500/20">
          ...
          <CardFooter className="justify-center border-t border-border/40 bg-muted/20 py-5 mt-8 px-6 sm:px-8">
            <p className="text-xs sm:text-sm text-muted-foreground font-semibold">
              New to AlgoSpark?{" "}
              <Link to="/signup" className="text-primary hover:text-primary/95 hover:underline font-bold inline-flex items-center gap-0.5">
                Create Account
